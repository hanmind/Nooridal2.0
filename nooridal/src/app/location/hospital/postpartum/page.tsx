...
export default function PostpartumCenterPage() {
  ...
  useEffect(() => {
    if (mapInstance && userCoords) {
      ...
      const userPosition = new window.kakao.maps.LatLng(userCoords.lat, userCoords.lng);
        ...
        const content = `
          <div style="padding:10px; width: 250px; font-size: 12px; line-height: 1.4;">
            ...
            <div style="margin-top: 5px;">
              정원: 산모 ${center.motherCapacity} / 영유아 ${center.infantCapacity}
            </div>
             ${center.nurseCount !== null ? `<div>간호사: ${center.nurseCount}명</div>` : ''}
             ${center.nurseAidCount !== null ? `<div>간호조무사: ${center.nurseAidCount}명</div>` : ''}
             ${center.distance !== undefined ? `<div>거리: ${center.distance.toFixed(2)}km</div>` : ''}
          </div>
        `;
  ...
  const displayCenters = userCoords ? nearbyCenters : allCenters.filter(c => c.lat && c.lng); // Show nearby if location known, else all with coords
  ...
  const resultsMessage = userCoords
    ...
    : `전체 ${allCenters.filter(c=>c.lat && c.lng).length}개 산후조리원 검색 결과 (위치 기반 필터링 불가):`;

  return (
    <div className="flex flex-col h-screen w-full max-w-md mx-auto bg-[#FFFAF0]"> {/* Changed background color */}
      {/* Header */}
      <div className="sticky top-0 z-10 flex items-center justify-center h-20 bg-white shadow-md rounded-b-2xl px-4">
        <button
          onClick={() => router.back()}
          className="absolute left-4 text-neutral-700 text-2xl"
          aria-label="뒤로 가기"
        >
          &lt;
        </button>
      ...
      {/* Map Area - Requires coordinates */}
      <div ref={mapContainerRef} className="w-full h-64 md:h-80 bg-gray-200 flex-shrink-0">
        {!mapLoaded && (
          <div className="flex items-center justify-center h-full">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-pink-500"></div> {/* Changed color */}
            <span className="ml-2 font-['Do_Hyeon'] text-gray-600">지도 로딩 중...</span>
          </div>
        )}
        {mapLoaded && allCenters.length > 0 && !allCenters.some(c => c.lat && c.lng) && (
            <div className="flex items-center justify-center h-full font-['Do_Hyeon'] text-gray-500">
                지도 표시에 필요한 좌표 정보가 없습니다.
            </div>
        )}
      </div>
      ...
      {loading ? (
        <div className="flex-grow flex items-center justify-center">
          ...
          <span className="ml-3 font-['Do_Hyeon'] text-gray-700">데이터 로딩 중...</span>
        </div>
      ) : (
          ...
          {displayCenters.length === 0 ? (
            ...
            <>
              ...
              {displayCenters.map(center => (
                <div key={center.id} className="bg-white p-3.5 rounded-xl shadow-sm border border-gray-200">
                  <div className="flex justify-between items-start">
                    <div className="flex-1 pr-3">
                      <h3 className="text-md font-['Do_Hyeon'] text-pink-700 mb-0.5">{center.name}</h3>
                      <p className="text-xs text-gray-700 mt-1.5 font-['Do_Hyeon']">{center.address}</p>
                       <p className="text-xs text-gray-500 mt-1 font-['Do_Hyeon']">
                          정원: 산모 {center.motherCapacity}명 / 영유아 {center.infantCapacity}명
                          {center.nurseCount !== null && ` | 간호사 ${center.nurseCount}명`}
                          {center.nurseAidCount !== null && ` | 조무사 ${center.nurseAidCount}명`}
                       </p>
                       {center.distance !== undefined && (
                         <p className="text-xs text-blue-600 font-bold font-['Do_Hyeon'] mt-1">거리: {center.distance.toFixed(2)} km</p>
                       )}
                    </div>
                    {/* Action buttons */}
                       ...
                       {center.phone && (
                        ...
                        >
                           <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.31l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.31-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                           </svg>
                          {center.phone}
                        </a>
                      )}
                      <a
                        href={`https://map.kakao.com/link/search/${encodeURIComponent(center.address)}`} // Search by address
                        target="_blank"
